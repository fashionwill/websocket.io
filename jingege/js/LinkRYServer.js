...
function setEmojiText(e){
    Emojitext = $(e).find(".rong-emoji-content").text();
    $("#ChatContenttext").val($("#ChatContenttext").val()+$(e).find("span").text());
}
...
function websocketfn(getthonk,getid,myret,userret){
	...
	$(".drawingEmoj").html(EmojiHtml);
	$(".drawingEmoj .rong-emoji-content").css({
	    "font-size":"1px !important;"
	})
	mui("#EmojViewScroll").scroll();
	...
	RongIMClient.setOnReceiveMessageListener({
		...
		onReceived: function (message) {
			...
			$(".chat-info-list").append('<li class="clearfix"><span class="userpic youPic" id="UserImg"></span><div class="UserCommInfo YouChatInfo">'+event+'</div></li>');
	...
	function sendTextMessage(event){
		...
		RongIMClient.getInstance().sendMessage(conversationtype, targetId, msg, {
	        onSuccess: function (message) {
	        	$(".chat-info-list").append('<li class="clearfix"><span class="userpic myPic" style="background:url('+myret.my_img+')"></span><div class="UserCommInfo MyChatInfo">'+event+'</div></li>');
	        	if($(".chat-info-list").outerHeight(true)>$(".scroll-wrap").outerHeight(true)){
			        var scrollh = $(".chat-info-list").outerHeight(true) - $(".scroll-wrap").outerHeight(true);
			        mui('#ChatViewScroll').scroll().scrollTo(0,-scrollh,0);
			    }
	...
	$(".more_ways").on("touchstart",function(){
		if($("#ChatContenttext").val() != ""){
			sendTextMessage($("#ChatContenttext").val());
		}else{
        ...
        function onloadPic(picUrl){
            ...
            RongIMClient.getInstance().sendMessage(conversationtype, targetId, msg, {
                onSuccess: function (message) {
                    ...
                    $(".chat-info-list").append(contentDiv);
                    if($(".chat-info-list").outerHeight(true)>$(".scroll-wrap").outerHeight(true)){
				        var scrollh = $(".chat-info-list").outerHeight(true) - $(".scroll-wrap").outerHeight(true);
				        mui('#ChatViewScroll').scroll().scrollTo(0,-scrollh,0);
				    }
